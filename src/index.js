...
const moduleAliases = packageJson._modularAliases;
if (!moduleAliases) {
    console.error(`_modularAliases in package.json is empty, skipping`);
    process.exit(0);
}
...
const LINK_ALIAS_PREFIX = '.modular-express-';
...
function getModuleNameFromAliasFile(aliasFileName) {
    ...
    const m = aliasFileName.match(new RegExp(`^\\${LINK_ALIAS_PREFIX}(.*)`)); // RegExp = /^\.modular-express-(.*)/
...
async function linkModule(moduleName) {
    ...
    if (moduleName.match(/^@/) && !packageJson._moduleAliasIgnoreWarning) {
        console.warn(
            chalk.yellow(`WARNING! Using @ in front of your module name ${moduleName} may cause data loss. Please read this issue thread https://github.com/Rush/modular-express/issues/3 and make a backup before executing any npm/yarn commands. The issue ultimately can be blamed on npm/yarn. You've been warned.`) +
...
async function linkModules() {
    ...
    console.log('modular-express:', modules.map(addColor).join(', '));
}
...
async function unlinkModules() {
    ...
    if (unlinkedModules.length) {
        console.log('modular-express: Cleaned ', unlinkedModules.filter(v => {
            return v.type !== 'none';
        }).map(addColorUnlink).join(' '));
    } else {
        console.log('modular-express: No modules to clean');
    }
}
